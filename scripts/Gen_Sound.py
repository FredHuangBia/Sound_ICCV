...
class Obj:
    ...
    def ReadObj(self,objId):
        ...
        if objId == 4:
            self.mass = 300
        self.objPath = os.path.join(self.ROOT,'data','%d'%self.objId,'%d.orig.obj'%self.objId)
    ...
    def WriteString(self):
        properties = ''
        properties += '%d %.4f '%(self.mass,self.collisionMargin)
...
if __name__ == '__main__':
    ...
    sceneConfig.read('./../config/scene/scene_%d.cfg'%sceneid)
    ...
    for objid in range(objnum):
        ...
        objs[objid].mass = sceneConfig.getint('Objects','obj_%d_is_active'%objid)
        objs[objid].Load()
    ...
    resultPath = os.path.join(ROOT,'result','scene-%d'%sceneid,objResultPath,matResultPath)
    ...
    with cd(simFilePath):
        if skip_bullet!=True:
            for obj in objs:
                subprocess.call('ln -s %s'%os.path.join(ROOT,'data','%d'%obj.objId,'%d.obj'%obj.objId)\
                                +' %d.obj'%obj.objId,shell=True)
                subprocess.call('ln -s %s'%os.path.join(ROOT,'data','%d'%obj.objId,'%d.orig.obj'%obj.objId)\
                                +' %d.orig.obj'%obj.objId,shell=True)

            subprocess.call('sh %s'%os.path.join(ROOT,'script','prepare_dat.sh'),shell=True)
        ...
        if (skip_sound!=True):
            for obj_id in range(objnum):
                ...
                subprocess.call('ln -s %s'%os.path.join(ROOT,'data','%d'%objs[obj_id].objId,'%d.vmap'%objs[obj_id].objId)\
                                +' %d.vmap'%objs[obj_id].objId,shell=True)
                subprocess.call('ln -s %s'%os.path.join(ROOT,'data','%d'%objs[obj_id].objId,\
                                                        'mat-%d'%objs[obj_id].matId,'moments')\
                                +' moments',shell=True)
                subprocess.call('ln -s %s'%os.path.join(ROOT,'data','%d'%objs[obj_id].objId,'%d.geo.txt'%objs[obj_id].objId)\
                                +' %d.geo.txt'%objs[obj_id].objId,shell=True)
                subprocess.call('ln -s %s'%os.path.join(ROOT,'data','%d'%objs[obj_id].objId,'%d.ev'%objs[obj_id].objId)\
                                +' %d.ev'%objs[obj_id].objId,shell=True)


                subprocess.call('sh %s'%os.path.join(ROOT,'script','prepare_ini.sh')+' -i %d '%obj_id\
    ...
    if skip_video!=True:
        ...
        for k in range(1,actobj_num+1):
            ffmpeg_audio_cmd+='-i continuous_audio%d.wav '%k
        ffmpeg_audio_cmd+= '-filter_complex amerge merged.wav -y'
