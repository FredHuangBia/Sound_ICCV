...
class Obj:
    ...
    def ReadPose(self,poseId):
        ...
        cfgPath = os.path.join(self.ROOT,'config','pose','pose_%d.cfg'%self.poseId)
    ...
    def ReadBulletSetup(self,bulletSetupId):
        ...
        cfgPath = os.path.join(self.ROOT,'config','bullet_setup','bullet_%d.cfg'%self.bulletSetupId)
    ...
    def ReadObj(self,objId):
        ...
        if self.active == 1:
            ...
            self.objPath = os.path.join(self.ROOT,'validation','%d'%self.objId,'%d.orig.obj'%self.objId)
        if self.active == 1:
            #volumefile = open(os.path.join(self.ROOT,'data/final100/%d/models/volume.txt'%self.objId))
            self.mass = 2 #float(volumefile.readline())*self.density/50
            print self.mass
        # Load mass
    ...
    def WriteShellCmd(self):
        cmd = '-n %d -d %f -A %1.5E -B %f'%(self.objId,self.density,self.alpha,self.beta)
        return cmd
...
class Cam:
    ...
    def __init__(self,cfgId=0):
        self.cfgId = cfgId
        self.cfgPath = os.path.join(self.ROOT,'config','camera','camera_%d.cfg'%cfgId)
        self.Load()
...
class Lighting:
    ...
    def __init__(self,cfgId=0):
        self.cfgId = cfgId
        self.cfgPath = os.path.join(self.ROOT,'config','lighting','lighting_%d.cfg'%cfgId)
    def SetCfgId(self,cfgId):
        self.cfgId = cfgId
        self.cfgPath = os.path.join(self.ROOT,'config','lighting','lighting_%d.cfg'%cfgId)
    def PrintPath(self):
...
if __name__ == '__main__':
    ...
    # READ SCENE CONFIG
    subprocess.call('bash /data/vision/billf/object-properties/sound/ztzhang/renew/re_new.sh',shell=True)
    if (len(argv)<2):
    ...
    sceneConfig.read(os.path.join(ROOT_DIR,'config/scene/scene_%d.cfg'%sceneid))
    ...
    if(len(argv)<(2+2*objnum)):
        ...
        for k in range(objnum):
            objs.append(Obj(objId=int(argv[k*2+2]),matId=int(argv[k*2+3])))
            
    for objid in range(objnum):
        objs[objid].poseId = (sceneConfig.getint('Objects','obj_%d_pose'%objid))
    ...
    simCfg.read(os.path.join(ROOT,'config','simulation','sim_%d.dat'%simId))
    ...
    resultPath = os.path.join(ROOT,'validation/result','scene-%d'%sceneid,objResultPath,matResultPath)
    ...
    with cd(simFilePath):
        if skip_bullet!=True:
            for obj in objs:
                ...
                if not os.path.exists('%d.obj'%obj.objId):
                    subprocess.call('ln -s %s'%os.path.join(ROOT,'validation','%d'%obj.objId,'%d.obj'%obj.objId)\
                                +' %d.obj'%obj.objId,shell=True)
                if not os.path.exists('%d.orig.obj'%obj.objId):
                    subprocess.call('ln -s %s'%os.path.join(ROOT,'validation','%d'%obj.objId,'%d.orig.obj'%obj.objId)\
                                +' %d.orig.obj'%obj.objId,shell=True)
                #print 'ln -s %s'%os.path.join(ROOT,'validation','%d'%obj.objId,'%d.obj'%obj.objId)
                #print 'ln -s %s'%os.path.join(ROOT,'validation','%d'%obj.objId,'%d.orig.obj'%obj.objId)
            subprocess.call('sh %s >log1.txt'%os.path.join(ROOT,'script','prepare_dat.sh'),shell=True)
        ...
        if (skip_sound!=True):
            for obj_id in range(0,objnum):
                ...
                subprocess.call('ln -s %s'%os.path.join(ROOT,'validation','%d'%objs[obj_id].objId,\
                ...
                if is_overwrite:
                    ...
                    if os.path.exists('./../obj-%04d.wav'%(obj_id)):
                        subprocess.call('rm ./../obj-%04d.wav'%(obj_id),shell = True)

                if not os.path.exists('./../%04d.wav'%(obj_id)) or not os.path.exists('./../%04d.raw'%(obj_id)) or is_overwrite:
                    print 'wav not generated yet, working on it !'
                    subprocess.call('rm *.wav',shell = True)
                    subprocess.call('rm *.raw',shell = True)
                    subprocess.call('bash %s'%os.path.join(ROOT,'script','prepare_ini.sh')+' -i %d '%obj_id\
                    ...
                    subprocess.call('mv *1.wav ./../%04d.wav'%(obj_id),shell=True)
                    subprocess.call('mv *1.raw ./../%04d.raw'%(obj_id),shell=True)
        ...
        if skip_rendering!=True and (not os.path.exists('./../sli.mp4') or is_overwrite):
            subprocess.call('sh /data/vision/billf/object-properties/sound/sound/script/unset.sh',shell=True)
            ...
            blenderScript = os.path.join(ROOT,'script','blender_render_scene.py')
    ...
    if skip_video!=True:
        ...
        if len(actobj_num)==1:
            ffmpeg_audio_cmd = 'cp obj-%04d.wav merged.wav'%(actobj_num[0].objId)
        else:
            for k in range(0,len(actobj_num)):
                ffmpeg_audio_cmd+='-i obj-%04d.wav '%(actobj_num[k].objId)
            ffmpeg_audio_cmd+= '-filter_complex -filter_complex amix=inputs=%d:duration=longest merged.wav -y'%(len(actobj_num))
