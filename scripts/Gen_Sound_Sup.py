...
class Obj:
    ...
    def ReadMaterial(self,matId):
        ...
        cfgPath = os.path.join(self.ROOT,'config','material','material-%d.cfg'%self.matId)
        print cfgPath
        matCfg.read(cfgPath)
    ...
    def ReadObj(self,objId):
        ...
        #    self.mass = 300
        if self.active == 1:
            self.objPath = os.path.join(self.ROOT,'data/ready','%d'%self.objId,'%d.orig.obj'%self.objId)
        else:
            self.objPath = os.path.join(self.ROOT,'data/ready','%d'%self.objId,'%d.orig.obj'%self.objId)
        if self.active == 1:
    ...
    def WriteShellCmd(self):
        cmd = '-n %d -d %f -A %1.5s -B %f'%(self.objId,self.density,self.alpha,self.beta)
        return cmd
...
if __name__ == '__main__':
    ...
    saveROOT = '/data/vision/billf/object-properties/sound/zhengjia/'
    resultPath = os.path.join(saveROOT,'result','scene-%d'%sceneid,objResultPath,matResultPath)
    ...
    with cd(simFilePath):
        if skip_bullet!=True:
            for obj in objs:
                ...
                #print 'ln -s %s'%os.path.join(ROOT,'data/ready','%d'%obj.objId,'%d.orig.obj'%obj.objId)
        ...
        if (skip_sound!=True):
            for obj_id in range(0,objnum):
                ...
                if is_overwrite:
                    ...
                    if os.path.exists('./../obj-%04d.wav'%(objs[obj_id].objId)):
                        subprocess.call('rm ./../obj-%04d.wav'%(objs[obj_id].objId),shell = True)

                if not os.path.exists('./../obj-%04d.wav'%(objs[obj_id].objId)) or not os.path.exists('./../obj-%04d.raw'%(objs[obj_id].objId)):
                    ...
                    subprocess.call('mv *1.wav ./../obj-%04d.wav'%(objs[obj_id].objId),shell=True)
                    subprocess.call('mv *1.raw ./../obj-%04d.raw'%(objs[obj_id].objId),shell=True)
        ...
        if skip_rendering!=True and (not os.path.exists('./../sli.mp4') or is_overwrite):
            ...
            blenderScript = os.path.join(ROOT,'script','blender_render_scene_texture.py')
