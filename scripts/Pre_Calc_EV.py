...
class Obj:
    ...
    def ReadObj(self,objId):
        self.objId = objId
        self.objPath = os.path.join(self.ROOT,'data/final100','%d'%self.objId,'models','%d.orig.obj'%self.objId)
    
    def ReadTet(self):
        self.tetPath = os.path.join(self.ROOT,'data/final100','%d'%self.objId,'models','obj-%d.tet'%self.objId)
...
if __name__=='__main__':
    ...
    objfilePath = os.path.join(ROOT_DIR,'data','final100','%d'%obj.objId,'models')
    outPath = os.path.join(ROOT_DIR,'data','final100','%d'%obj.objId,'models','mat-%d'%obj.matId)
    CreateDir(outPath)
    ...
    #call ev calculation, save to mat-id
        ...
        if not os.path.exists('obj-%d.ev'%obj.objId) or overwrite==1:
            ...
            cmd = 'matlab -nodisplay -nodesktop -nosplash -r "addpath(\'%s\'); ev_generator60(\'%s\', 60); quit"| tee -a %s'%(FILEGENERATORS,'obj-%d'%obj.objId,logfile)
            subprocess.call(cmd ,shell=True)
